...
Every tool call fires a small same-origin beacon (:js:`navigator.sendBeacon`,
with a ``fetch`` fallback) to the ingest endpoint. The
:php:`\Neoblack\Webmcp\Middleware\EventMiddleware` appends exactly two values per
call:

*   the **tool name** – validated against the registered providers
    (:php:`\Neoblack\Webmcp\Registry\ToolRegistry::toolNames()`); events for
...
..  note::
    ...
    Nothing else is stored: no free text (search queries, e-mail contents,
    names), no cookies, no IP address, no user agent string.
...
#.  the agent's optional ``client`` argument, if it sets one (the runtime adds a
    ``client`` property to every tool's input schema automatically);
#.  otherwise a coarse match against known agent markers in the User-Agent
    (e.g. ``Claude``, ``ChatGPT``, ``GeminiBot``);
#.  otherwise the literal ``unbekannt`` ("unknown").
...
Events live in a single append-only table, created by :bash:`extension:setup`:

..  list-table:: Table ``tx_neoblackwebmcp_event``
    :header-rows: 1
    :widths: 20 25 55
...
..  code-block:: sql
    :caption: Trim or clear the usage log
...
..  list-table::
    ...
    *   -   Constraint
        -   Behaviour
    *   -   Feature-gated
        -   When ``analyticsEnabled`` is off the endpoint is inert and the
            request is passed through untouched.
    *   -   Same-origin guard
        -   Requests whose ``Sec-Fetch-Site`` header is present and neither
            ``same-origin`` nor ``same-site`` are rejected with ``204``.
    *   -   Rate limited
        -   At most ``analyticsRateLimit`` calls per client IP per minute
    ...
    *   -   Whitelisted
        -   Only registered tool names are logged; anything else is handed down
...
timeframe (7 / 30 / 90 days) and tool/client filters are selectable; the accepted
tool names are derived automatically from the registered providers.
...
..  seealso::
    ...
    *   :ref:`configuration` – switches for ``analyticsEnabled`` and
        ``analyticsRateLimit``.
    *   :ref:`architecture` – how the ingest flow fits together.
